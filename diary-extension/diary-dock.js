...
(function() {
  ...
  function buildDock() {
    const PROVIDERS = [
      ...
      { id: 'meta',       url: 'https://www.meta.ai',            label: 'Meta AI',    dot: 'fgd-dot-meta',       active: false },
    ];
    // Detect current host
    ...
    PROVIDERS.forEach(p => { 
      p.active = host.includes(p.id) || 
        ...
        (p.id === 'meta' && host.includes('meta.ai'));
    });
    ...
    // Always-visible Diary toggle on the tab
    const diaryToggle = document.createElement('div');
    diaryToggle.id = 'diary-toggle';
    diaryToggle.title = 'Open Diary';
    diaryToggle.textContent = String.fromCharCode(55357,56613);
    diaryToggle.style.cssText = 'position:fixed;right:0;top:calc(50% + 80px);width:40px;height:36px;background:#1B2A4A;border:1px solid rgba(193,125,60,0.4);border-right:none;border-radius:8px 0 0 8px;display:flex;align-items:center;justify-content:center;font-size:16px;cursor:pointer;z-index:2147483641;transition:all 0.2s;';
    diaryToggle.onmouseenter = function() { this.style.background = 'rgba(193,125,60,0.15)'; this.style.borderColor = '#C17D3C'; };
    diaryToggle.onmouseleave = function() { this.style.background = '#1B2A4A'; this.style.borderColor = 'rgba(193,125,60,0.4)'; };
    diaryToggle.onclick = function(e) { e.stopPropagation(); window.open('https://diary.projectcoachai.com/app.html', '_blank'); };
    document.body.appendChild(diaryToggle);
    tab.setAttribute('aria-label', 'Open Forge provider switcher');
    tab.setAttribute('aria-expanded', 'false'); tab.tabIndex = 0;
    const tabMark = document.createElement('div'); tabMark.className = 'fgd-mark'; tabMark.setAttribute('aria-hidden','true'); tabMark.textContent = 'D';
    ...
    mark.style.cssText = 'width:22px;height:22px;font-size:12px;border-radius:6px;'; mark.textContent = 'D';
    ...
    persp.href = 'https://diary.projectcoachai.com'; persp.textContent = '✦ All Perspectives';
    persp.addEventListener('click', function(e) {
      e.preventDefault();
      try { chrome.runtime.sendMessage({ type: 'SWITCH_PROVIDER_TAB', url: 'https://diary.projectcoachai.com' }, (res) => { if (!res || !res.switched) window.location.href = 'https://diary.projectcoachai.com'; }); }
      catch(_) { window.location.href = 'https://diary.projectcoachai.com'; }
    });
    ...
    excel.addEventListener('click', function() {
      try { chrome.runtime.sendMessage({ type: 'SWITCH_PROVIDER_TAB', url: 'https://diary.projectcoachai.com/excel.html' }, (res) => { if (!res || !res.switched) window.location.href = 'https://diary.projectcoachai.com/excel.html'; }); }
      catch(_) { window.location.href = 'https://diary.projectcoachai.com/excel.html'; }
    });
    ...
    split.addEventListener('click', function() {
      ...
      window.postMessage({ type: '__DIARY_TO_EXT__', payload: { type: 'OPEN_SPLIT_WINDOW' } }, '*');
    });
    ...
    const upg = document.createElement('a'); upg.href = 'https://diary.projectcoachai.com/pricing.html'; upg.textContent = 'Upgrade';
    const sep = document.createTextNode(' · ');
    const hlp = document.createElement('a'); hlp.href = 'https://diary.projectcoachai.com/help.html'; hlp.textContent = 'Help';
  ...
  const FORGE_HOME = 'https://diary.projectcoachai.com';

  /* ── ELEMENTS — query inside shadow DOM ── */
  const dock  = shadow.getElementById('diary-dock');
