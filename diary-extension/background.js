...
async function fetchSyncResultFromTab(tabId, conversationUrl) {
  ...
  while (result && !result.success && retryCount < saveRetryMax) {
    ...
    // 'provider_not_authenticated' added as retryable — a real, separate
    // case from 'not_authenticated' above (that one is about being
    // logged into Diary itself; this one is about being logged into the
    // AI provider, confirmed live for ChatGPT specifically: navigating
    // to a conversation URL while logged out redirects to the provider's
    // own plain homepage instead). Retryable for the same underlying
    // reason — a real chance the person logs into the provider during
    // this same retry window — but reports a distinct, specific error
    // code once every attempt is genuinely exhausted, so the person
    // gets a real, actionable message instead of the same generic
    // "no content found" a transient rendering delay would also produce.
    var retryableErrors = result.error === 'no_content_found' || result.error === 'incomplete_response' || result.error === 'timeout' || result.error === 'not_authenticated' || result.error === 'provider_not_authenticated';
    ...
    (async function() {
      ...
      try {
        ...
        if (existingTab) {
          ...
          // Confirmed as a real, direct bug: directly reported live —
          // a person who had just manually logged in using this exact
          // tab (still the active, visible tab in their browser at that
          // moment) then triggered sync, which reused this same tab
          // without ever explicitly backgrounding it first, unlike the
          // new-tab path below (which is always created with
          // active:false, backgrounded from the very start by
          // construction). This is exactly why the same conversation's
          // sync flashed the first time (fresh, correctly-backgrounded
          // tab) but not the second (a reused tab left in whatever
          // active/visible state it already happened to be in).
          // Explicitly backgrounds a reused tab the same way a fresh
          // one already is, so this behaves consistently regardless of
          // whether a new or existing tab ends up being used.
          try { await chrome.tabs.update(tabId, { active: false }); } catch (_e) {}
        } else {
