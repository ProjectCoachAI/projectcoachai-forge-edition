...
function queryAllDeep(selector) {
  ...
  function injectSaveDiaryButton(responseText) {
    ...
    // Extracted from what was previously the inline body of
    // performSaveToDiary's own ChatGPT clipboard-copy branch, per the
    // Layer 1 capture-stability redesign — see that call site's own
    // comment for the full rationale. Behavior is IDENTICAL to the
    // original inline version; only the packaging changed, so it can be
    // called twice (or more) and compared, instead of running once and
    // being trusted immediately. Returns { success, fullThread,
    // turnCount, prompt } rather than mutating outer-scope variables
    // directly, so two separate attempts can be compared side by side
    // before either one is committed to.
    ...
    async function performSaveToDiary(btn) {
      ...
      try {
        ...
        // which each had a real, evidenced failure mode. This is the
        ...
        //
        // REVERTED, isolated entirely within ChatGPT's own branch, after a
        // direct, live re-test of the history-JSON-first approach above
        // (kept temporarily to test the August 8 rebuild's own reasoning
        // for abandoning it): a real conversation's SECOND sync hit 25
        // failed retries over ~325 seconds before timing out completely —
        // a full, hard failure, not a mismatch or "no new content", and
        // worse than the visible flash it was meant to remove. This
        // directly confirms the original rebuild's own stated reasoning
        // (DOM virtualization / backend propagation delay making
        // history-JSON genuinely unreliable for ChatGPT) still holds
        // today, not just historically. Isolating this test to ChatGPT's
        // own branch meant this real, hard failure never had any chance
        // to reach DeepSeek, Meta AI, or Claude while it was live.
        //
        ...
        if (PROVIDER === 'chatgpt') {
          ...
          // Preventative rather than corrective — per explicit direction
          // after three same-day backend comparison bugs (a citation
          // link, a Sources-footer relocation, a Sources-footer
          // occurrence count) were each traced to the same root fact:
          // ChatGPT's own captured text isn't guaranteed byte-identical
          // across two separate reads of an unchanged conversation, and
          // no amount of teaching the comparison logic to ignore the
          // NEXT way it can cosmetically vary actually closes that gap
          // for good. This runs the exact same clipboard-copy capture
          // TWICE, with a short wait between, and only treats the result
          // as final once two consecutive captures agree (after the same
          // normalization already applied server-side: URL/bold-marker/
          // Sources-footer stripped) — directly targeting ChatGPT's own
          // documented backend propagation delay (a capture catching an
          // answer before ChatGPT's backend has actually finished
          // persisting it) at the SOURCE, rather than detecting its
          // aftermath downstream in the merge logic, the same way the
          // history-fetch fallback below already does for itself.
          var chatgptStabilityNormalize = function(s) {
            return (s || '').replace(/\n\n---\n\n\*\*Sources:\*\*\n[\s\S]*$/, '').replace(/https?:\/\/\S+/g, '').replace(/\*\*/g, '').replace(/\s+/g, ' ').trim();
          };
          var chatgptCaptureAttempt = null;
          var chatgptStableAfterAttempts = null;
          for (var stabilityAttempt = 0; stabilityAttempt < 3; stabilityAttempt++) {
            var thisAttempt = await attemptChatGPTClipboardCapture();
            if (chatgptCaptureAttempt && thisAttempt.success && chatgptCaptureAttempt.success &&
                chatgptStabilityNormalize(thisAttempt.fullThread) === chatgptStabilityNormalize(chatgptCaptureAttempt.fullThread)) {
              chatgptStableAfterAttempts = stabilityAttempt + 1;
              chatgptCaptureAttempt = thisAttempt;
              break;
            }
            chatgptCaptureAttempt = thisAttempt;
          ...
          if (chatgptStableAfterAttempts) {
            console.log('[Diary] ChatGPT capture confirmed STABLE after', chatgptStableAfterAttempts, 'consecutive matching attempt(s)');
          } else {
          ...
          if (chatgptCaptureAttempt && chatgptCaptureAttempt.success) {
            chatgptClipboardWorked = true;
            fullThread = chatgptCaptureAttempt.fullThread;
            turnCountForSync = chatgptCaptureAttempt.turnCount;
            if (chatgptCaptureAttempt.prompt) prompt = chatgptCaptureAttempt.prompt;
          }
        }
