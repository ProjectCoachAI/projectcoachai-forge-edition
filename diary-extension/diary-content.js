...
function queryAllDeep(selector) {
  ...
  // buildDomPairedThread() and buildGeminiPairedThread() now live in
  // diary-content-dom-parsing.js (loaded alongside this file, per
  ...
  function injectSaveDiaryButton(responseText) {
    ...
    // Extracted into diary-content-chatgpt.js as part of a deliberate,
    // architectural simplification pass — see that file's own opening
    // comment for the full rationale. Both attemptChatGPTClipboardCapture()
    // and its own stability-check wrapper (runChatGPTCaptureWithStability(),
    // called directly below) now live there; behavior is UNCHANGED, only
    // the packaging moved — both files are loaded together into the same
    // isolated-world global scope (see manifest.json), so this call site
    // needed no change beyond what's directly below.
    ...
    async function performSaveToDiary(btn) {
      ...
      try {
        ...
        if (PROVIDER === 'chatgpt') {
          // Delegates to diary-content-chatgpt.js's own
          // runChatGPTCaptureWithStability() — see that file's own
          // comment for the full rationale on the capture-stability
          // check itself. Behavior is UNCHANGED from the previous inline
          // version; only the packaging moved as part of a deliberate,
          ...
          if (chatgptCaptureResult && chatgptCaptureResult.success) {
            ...
            turnCountForSync = chatgptCaptureResult.turnCount;
            if (chatgptCaptureResult.prompt) prompt = chatgptCaptureResult.prompt;
          }
        }
