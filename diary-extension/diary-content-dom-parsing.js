...
//
// UPDATE: buildDomPairedThread() below is NO LONGER CALLED BY ANY
// PROVIDER — kept in place only as historical reference / potential
// future scaffolding, not because anything depends on it. It was
// originally shared across four separate providers (Perplexity, Meta
// AI, Grok, Mistral, each passing their own selector configuration
// into the same generic function). After a direct, live comparison
// against Gemini — which has always had its own dedicated function,
// never shared with anyone, and has never shown the pairing/ordering
// problems the four shared-function providers did across this session
// — the decision was made to stop sharing entirely: each of those four
// providers now has its own standalone, non-shared function
// (buildGrokPairedThread, buildPerplexityPairedThread,
// buildMetaAIPairedThread, buildMistralPairedThread, each defined
// further below), matching Gemini's own proven structure exactly. A
// future fix for one provider's specific DOM quirk can now never
// silently affect another, by construction — the entire point of the
// split. Each of the four new functions currently contains the exact
// same logic as this one (a straight copy at the moment of the split,
// not yet independently modified) — see each one's own location for
// where to make provider-specific changes from now on.
//
...
// dependencies these functions have -- boldQuestion(),
...
// diary-content-chatgpt.js.
  ...
  // ── Provider-specific dedicated functions ────────────────────────────────
  // Following the same architectural pattern as buildGeminiPairedThread
  // below (one dedicated function per provider, tuned only for that
  // provider's own confirmed DOM, never shared with any other provider):
  // these four functions are exact standalone copies of the logic above
  // (buildDomPairedThread), one per provider that previously called that
  // single shared function through different configuration options.
  //
  // Rationale, decided directly after a live comparison: Gemini (working,
  // reliably, every test) has always had its own dedicated function,
  // never shared with anyone. Grok, Perplexity, Meta AI, and Mistral
  // (all showing real, evidenced problems across this session) all
  // shared ONE generic function through configuration — meaning any
  // future fix made for one provider's specific DOM quirk risked
  // silently changing behavior for the other three sharing the same
  // code path. Splitting into genuinely separate, non-shared functions
  // means a change to one can never again affect another, by
  // construction — matching Gemini's own proven structure exactly.
  //
  // Each function's logic is currently identical to the original shared
  // buildDomPairedThread (a straight copy, not yet independently
  // modified) — this commit is purely the architectural split itself.
  // Any future provider-specific fix should be made ONLY inside that
  // provider's own function from now on.

  function buildGrokPairedThread(opts) {
    try {
      var els = document.querySelectorAll(opts.combinedSelector);
      if (!els.length) return null;
      var parts = [];
  ...
  function buildPerplexityPairedThread(opts) {
    try {
      var els = document.querySelectorAll(opts.combinedSelector);
      if (!els.length) return null;
      var parts = [];
  ...
  function buildMetaAIPairedThread(opts) {
    try {
      var els = document.querySelectorAll(opts.combinedSelector);
      if (!els.length) return null;
      var parts = [];
  ...
  function buildMistralPairedThread(opts) {
    try {
      var els = document.querySelectorAll(opts.combinedSelector);
      if (!els.length) return null;
      var parts = [];
