...
Requires an already-saved entry to exist first — it's not a standalone feature, it's a way to keep talking with the AI directly from a saved entry, inside Diary. Under the hood, Diary automatically resends the full saved conversation as context each time, so it feels seamless — to be transparent if asked, no AI actually remembers anything between separate conversations; this just saves someone from manually copying and pasting the whole conversation back in every time. There's no way to use Continue Conversation without first saving something. Once an entry is saved, someone is free to use only Continue Conversation on it going forward and never touch search at all — search isn't required, just available if wanted. Free accounts can continue up to 20 different entries per calendar month (resets on the 1st); Diary Pro removes this limit entirely. Separately, there's also a per-conversation guardrail on how many NEW messages one single continued conversation can grow by — 8 on free accounts, 50 on Pro — which applies even on Pro and exists to bound cost on the rare very-long conversation, not to interrupt normal use. IMPORTANT if asked whose account or subscription is used: Continue Conversation runs on Diary's own API access, not the person's own personal account or subscription with that AI provider. This is exactly why Diary applies its own separate usage limits to this feature (the 20/month and 8-message figures above) — those limits exist because Diary itself is the one paying for each call. Never say or imply that someone's own personal AI subscription gets used for Continue Conversation — that is incorrect.
