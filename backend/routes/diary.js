...
// Sync/merge logic, and the message-splitting helper it depends on,
// live in their own module now — see that file's own opening comment
// for the full rationale (a deliberate, architectural simplification
// pass extracting the single largest block from what had grown into a
// ~2,800-line route file).
const { splitEntryIntoMessages, performChatSessionSync } = require('./diary-sync');
...
// splitEntryIntoMessages() now lives in ./diary-sync (imported above) —
// see that file's own opening comment for why.


// ── Semantic search: Voyage AI embeddings ───────────────────────────────────
// Moved to backend/lib/embeddings.js — now shared with chat.js's own
// conversation-bridging logic, which needs the exact same embedding
// model/dimensionality. See that file's own comment for the full
// reasoning (unchanged from what was originally here).
...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    // way by default, not deleted). ?archived=true shows ONLY archived
    // entries (the "Archived" view). ?archived=all disables this filter
    // entirely, showing everything regardless of archived state — useful
    // for search, which should probably still find archived content.
    if (archived === 'true') {
      whereSql += ` AND is_archived = true`;
    } else if (archived !== 'all') {
      whereSql += ` AND (is_archived = false OR is_archived IS NULL)`;
    }
    ...
    const sql = `SELECT id, source, title, prompt, content, document_text,
                        conversation, decision_note, category, tags, metadata, rating, is_favorite, is_archived, archived_at, conversation_count, open_count, created_at, updated_at, artifacts
...
router.post('/:id/read-aloud', requireAuth, async (req, res) => {
  try {
    ...
    // Cache lookups run in parallel (Promise.all), not one at a time —
    // confirmed as a real, direct contributor to a real user report of
    // the Listen button appearing stuck: a longer diary entry splitting
    // into several chunks was checking (and, below, generating) each
    // one sequentially, multiplying real latency by the chunk count.
    const cachedUrls = await Promise.all(chunkHashes.map(async hash => {
      const cacheRes = await db.query('SELECT audio_url FROM tts_cache WHERE entry_id=$1 AND content_hash=$2', [id, hash]);
      return cacheRes.rows.length ? cacheRes.rows[0].audio_url : null;
    }));

    // A full cache hit (every chunk already generated) is a genuinely
    // free replay with zero new API cost, per the brief's own explicit
    // acceptance criterion — checked, and correctly skips usage
    // metering entirely, BEFORE even checking eligibility at all.
    ...
    try {
      audioUrls = await Promise.all(chunks.map(async (chunkText, i) => {
        if (cachedUrls[i]) return cachedUrls[i];
        const audioBuffer = await readAloudTts.generateSpeech(chunkText);
...
router.patch('/:id', requireAuth, async (req, res) => {
  try {
    ...
    // implementing here.
    const { content, metadata, prompt, category, decision_note, rating, is_favorite, is_archived, turnCount } = req.body;
    ...
    // Sync/merge logic (chat_sessions comparison and merge) now lives in
    // ./diary-sync's own performChatSessionSync() — see that module's
    // own opening comment for the full rationale. Behavior is UNCHANGED
    // from the previous inline version; only the packaging moved.
    let chatSessionSyncResult = null;
    if (content !== undefined) {
      const syncResult = await performChatSessionSync({ id, content, prompt, turnCount, userEmail: req.userEmail });
      chatSessionSyncResult = syncResult.chatSessionSyncResult;
      // willWriteContentDirectly is true in exactly the two cases the
      // original inline block also wrote content/search_text directly:
      // a pre-fork entry (no chatSessionId yet), or content sent for an
      // id that turns out not to exist at all. The actual sets.push()/
      // params.push() calls stay here rather than moving into
      // performChatSessionSync() itself, since they depend on this
      // route's own $${i++} positional counter -- a value specific to
      // this request's own SET-clause construction, not something a
      // separate module can safely own.
      if (syncResult.willWriteContentDirectly) {
    ...
    if (is_archived !== undefined) {
      sets.push(`is_archived=$${i++}`); params.push(is_archived);
      // archived_at set on archive, cleared on unarchive — so the
      // timestamp always reflects the most recent archive action, and
      // is NULL for anything that isn't currently archived, avoiding a
      // stale "archived_at" surviving an unarchive/re-archive cycle
      // with a misleadingly old date.
      sets.push(`archived_at=$${i++}`); params.push(is_archived ? new Date().toISOString() : null);
    }
