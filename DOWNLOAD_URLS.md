# ProjectCoachAI Forge Edition V1.0.0 - Download URLs

## Release Information

- **Release Tag**: V1.0.0
- **Release Title**: ProjectCoachAI Forge Edition V1 - Initial Release
- **Release URL**: https://github.com/ProjectCoachAI/projectcoachai-forge-edition/releases/tag/V1.0.0
- **Status**: ✅ Published (Latest Release)

---

## Direct Download Links

### macOS Installer

**File**: `ProjectCoachAI.Forge.Edition.V1-1.0.0-arm64.dmg`
**Size**: 89.8 MB
**SHA256**: `444f05fc65b3edd6ced57c7e82...`

**Direct Download URL:**
```
https://github.com/ProjectCoachAI/projectcoachai-forge-edition/releases/download/V1.0.0/ProjectCoachAI.Forge.Edition.V1-1.0.0-arm64.dmg
```

### Windows Installer

**File**: `ProjectCoachAI.Forge.Edition.V1.Setup.1.0.0.exe` (GitHub renamed spaces to dots)
**Size**: 146 MB
**SHA256**: `a8f18a6da7eb95b16546f8780f...`

**Direct Download URL:**
```
https://github.com/ProjectCoachAI/projectcoachai-forge-edition/releases/download/V1.0.0/ProjectCoachAI.Forge.Edition.V1.Setup.1.0.0.exe
```

⚠️ **NOTE**: GitHub automatically renames files with spaces to use dots when uploaded via web interface. Use the dots version in URLs.

---

## For Your Website (projectcoachai.com)

### Option 1: Direct Links to GitHub Releases

**HTML Code for Download Buttons:**

```html
<!-- macOS Download Button -->
<a href="https://github.com/ProjectCoachAI/projectcoachai-forge-edition/releases/download/V1.0.0/ProjectCoachAI.Forge.Edition.V1-1.0.0-arm64.dmg" 
   class="btn btn-primary" 
   download="ProjectCoachAI-Forge-Edition-V1-1.0.0.dmg">
  Download for macOS (89.8 MB)
</a>

<!-- Windows Download Button -->
<a href="https://github.com/ProjectCoachAI/projectcoachai-forge-edition/releases/download/V1.0.0/ProjectCoachAI.Forge.Edition.V1.Setup.1.0.0.exe" 
   class="btn btn-primary" 
   download="ProjectCoachAI-Forge-Edition-V1-Setup-1.0.0.exe">
  Download for Windows (146 MB)
</a>
```

### Option 2: Link to Release Page

**Link to entire release page:**

```html
<a href="https://github.com/ProjectCoachAI/projectcoachai-forge-edition/releases/tag/V1.0.0" 
   class="btn btn-primary">
  Download ProjectCoachAI Forge Edition V1.0.0
</a>
```

---

## Next Steps

1. ✅ **Release Published** - Done!
2. ✅ **Test Downloads** - Download both files and test installation
3. ✅ **Update Website** - Add download links to projectcoachai.com
4. ✅ **Clean Up Repository** - Remove unwanted files (after testing)
5. ✅ **Launch Announcement** - Ready for February 1 launch!

---

**Release is live! Ready for testing and website updates!** 🚀
